...
router.get("/list-admins", async (req, res) => {
  try {
    ...
    let user;
    req.query.name ?
      user = await User.find({ deleted: { $ne: true }, first_name: { $regex: req.query.name } }).skip(per_page * page - per_page).limit(50)
      : user = await User.find({ deleted: { $ne: true } }).sort({ _id: -1 }).skip(per_page * page - per_page).limit(50)
    res.status(200).json({
      error: false,
      data: user,
    });
...
router.delete("/delete-admin/:admin_id", async (req, res) => {
...
router.put("/edit-subAdmin/:id", async (req, res) => {
  let data = req.body;
  const schema = Joi.object({
    email: Joi.string(),
    first_name: Joi.string(),
